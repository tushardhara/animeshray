		...
		if ( $holder ) {
			...
			if ( $is_self_hosted ) {
				...
				if ( $is_youtube ) {
					...
					printf( '<iframe class="thb-video-api thb-video-youtube" id="%s" src="//www.youtube.com/embed/%s?enablejsapi=1&amp;modestbranding&amp;showinfo=0&amp;rel=0%s"></iframe>', $id, $code, $params );
				}
				elseif( $is_vimeo ) {
					if ( $loop ) {
						$params .= '&amp;loop=1';
